...
Release verification answers whether a deploy worked. It cannot answer whether
production is still working an hour later, because it only runs when a deploy
happens — and the failures that matter here arrive on their own schedule.

The service runs as a single replica with a capped restart policy. Once the
retry budget is spent, a crash-loop stays down permanently, and the causes of
that state — an exhausted volume, a rotated credential, a bad migration — have
nothing to do with merges. Without continuous observation, an outage lasts until
the next deploy or the next scheduled job, whichever comes first.

Two properties make the observation trustworthy rather than merely present. It
must read the origin, because a cached response outlives the instance that
produced it and reports health that may no longer exist. And its alert must be
specific to this concern, because an alert another operation can close is an
alert that can be wrong about the present.
